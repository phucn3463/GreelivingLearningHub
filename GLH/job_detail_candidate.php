...
$checkQuery = "SELECT id FROM tbljobapplication WHERE jobId = $jobId AND candidateId = $candidateId";
$checkResult = mysqli_query($conn, $checkQuery);

if ($checkResult && mysqli_num_rows($checkResult) > 0) {
    // Job application exists, redirect to the job application detail page
    $jobApplicationRow = mysqli_fetch_assoc($checkResult);
    $jobApplicationId = $jobApplicationRow['id'];
    header("Location: job_application_detail_candidate.php?jobApplicationId=$jobApplicationId");
    exit();
}
...
<body>
    ...
    ?>
    <main class='candidatecoursedetail'>
        <h1>Job's detail</h1>
        ...
        <div>
            <p class='applybutton'><a href="apply_job.php?jobId=<?php echo $jobQueryRow['jobId']; ?>">Apply for this job</a><p>
        </div>

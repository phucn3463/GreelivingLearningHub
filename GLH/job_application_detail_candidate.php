...
$jobQuery = "SELECT
            ...
            tbljobapplication.status AS applyStatus,
            tbljobapplication.cv AS applyCv
            FROM
            tbljob
            ...
            WHERE
            tbljobapplication.id = $jobApplicationId;";
...
<body>
    ...
    ?>
    <main class='jobapplicationdetailcandidate'>
    <h1>Job's detail</h1>
    
    <div>
        <h2>Title: <?php echo $jobQueryRow['jobTitle']; ?></h2>
        <p>Specialization: <?php echo $jobQueryRow['jobSpec']; ?></p>
        <p>CV: <a href="uploads/<?php echo $jobQueryRow['applyCv']; ?>" target="_blank" class='viewcvbuttton'>View CV</a></p>
        <p>Status: <?php echo $jobQueryRow['applyStatus']; ?></p>
        <?php
            if ($jobQueryRow['applyStatus'] == 'Awaiting Interview') {
                $appointmentQuery = "SELECT * FROM `tbljobapplicationappointment` WHERE jobApplicationId = $jobApplicationId;";
                $appointmentQueryResult = mysqli_query($conn, $appointmentQuery) or die('Appointment query failed');
                $appointmentQueryRow = mysqli_fetch_assoc($appointmentQueryResult);

                echo "Appointment: " . $appointmentQueryRow['appointmentStart'] . " - " . $appointmentQueryRow['appointmentEnd'];
            }
